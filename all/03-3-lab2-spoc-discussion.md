...
```
 ...
 ```
- 初始化物理内存管理器，使其能够分配和回收物理内存并采用不同的分配策略。
- 探测物理内存，保留已经被使用的物理内存并创建一个空间的物理页列表。
- 检测物理内存管理器的分配函数是否正常工作。
- 创建二级页表并对其进行检查
- 建立虚拟页表与物理页表的对应关系。
- 建立和取消临时的基于段机制的映射关系。
- 初始化GDT表，写入段描述符并将GDT起始地址写入GDTR寄存器。
...
- qemu运行过程中出现了triple fault异常，所以qemu启动shutdown cycle关闭系统。
- 产生错误的原因是操作系统没有正确建立好IDT，系统没有办法正确调用中断handle或者double fault handler。
...
- 2011011237 mod 37 = 2, 因此加入"int $2"语句之后得到下列结果。有差异。
...
错误是因为中断号为2的中断并没有被指定中断处理例程，在IDT指定的位置找不到相应的中断处理程序。
