...
```
 ...
 ```
- 调用了pic_init函数，初始化8259A中断控制器，包括各个标志位和掩码的初始化设置
- 调用了idt_init函数，对所有中断入口进行了设置，包括了时钟中断的中断描述符和相应的偏移量
- 调用了clock_init函数，设置了8253时钟芯片的相应初始值，将时钟ticks初始为零，然后设置中断控制器中的时钟中断
- 调用了intr_enable函数，执行sti汇编命令，操作系统开始提供中断服务
...
- 实现可显示字符串的bootloader，包括对CGA、串并口的操作，切换实模式到保护模式、创建GDT，加载Kernel等
- 实现ELF文件格式的解析，使能中断，建立IDT
